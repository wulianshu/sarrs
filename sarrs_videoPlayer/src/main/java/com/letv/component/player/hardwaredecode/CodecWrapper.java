...
public class CodecWrapper{
	...
	{
		Log.d(LOG_TAG, "start create decoder, videoWidth:"+String.valueOf(videoWidth)+
				"videoHeight:"+String.valueOf(videoHeight));
		...
		*/
	    ...
	    try{
		    format = MediaFormat.createVideoFormat("video/avc", videoWidth, videoHeight );
	...
	public int fillInputBuffer( byte[] data, long pts, int flush ) 
	{
	    ...
	    if( flush == 999 )
	    {
	    	Log.d( LOG_TAG, "fillInputBuffer to flush" );
			int i = 0;
			for(i=0; i< codecInputBuffers.length; i++)
			{
				codecInputBuffers[i].clear();
	    ...
	    */
	    ...
	    try{
		    ...
		    int inputBufIndex = codec.dequeueInputBuffer( 10000 );//-1 means blocking here until buffer is available kTimeOutUs*100
		    Log.d( LOG_TAG, "InputBuf res:"+String.valueOf(inputBufIndex) + "  pts:"+String.valueOf(pts) + 
		    		"  datasize:"+String.valueOf(sampleSize));
		    if( inputBufIndex >= 0 )
		    {
		        ...
		        //Log.d( LOG_TAG, "presentationTimeUs:"+String.valueOf( presentationTimeUs ) );
				if(first_frame == 0)
				{
					first_frame = 1;
...
//			        codec.flush();
	...
	public int getOutputBuffer( ) 
	{
	    ...
	    final int res = codec.dequeueOutputBuffer( info, 10000 );//-1 kTimeOutUs*10
	    Log.d(LOG_TAG, "getOutputBuffer res="+String.valueOf( res ));
	    if( res >= 0 )
	    {
	        ...
	        //Log.d( LOG_TAG,"pts in info is: "+String.valueOf( info.presentationTimeUs ));
	        if( info.size > 0 )
	        {
	            //do the things
	        	/*
	            ...
	            {
		            try {
		            	File file = new File("/data/data/com.qiyi.video/files/dec.dec");
...
//	        MediaFormat format = codec.getOutputFormat( );
	        //Log.d( LOG_TAG, "format width:" + format.getInteger(MediaFormat.KEY_WIDTH)+" format height:" + format.getInteger(MediaFormat.KEY_HEIGHT) );
	        ...
	        if( mPlayer!= null )
	        {
	        	try {
	        		...
	        		//Log.d(LOG_TAG, "call sync with pts:"+String.valueOf(info.presentationTimeUs)+
	    ...
	    else if( res == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED )
	    {
	        ...
	        codecOutputBuffers = codec.getOutputBuffers( );
	        Log.d( LOG_TAG, "INFO_OUTPUT_BUFFERS_CHANGED buffer length:"+String.valueOf(codecOutputBuffers.length) );
	    ...
	    else if( res == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED )
	    {
	        ...
	        Log.d( LOG_TAG, "Output format width:" + oformat.getInteger(MediaFormat.KEY_WIDTH) );
	        Log.d( LOG_TAG, "Output format height:" + oformat.getInteger(MediaFormat.KEY_HEIGHT) );
	        return OUTPUT_FORMAT_CHANGED;
	        /*
	        if( mWidth != oformat.getInteger(MediaFormat.KEY_WIDTH) ||
	        		mHeight != oformat.getInteger(MediaFormat.KEY_HEIGHT) )
	        {
		        Log.d( LOG_TAG, "Need to reset codec" );
		        mWidth = oformat.getInteger(MediaFormat.KEY_WIDTH);
		        mHeight = oformat.getInteger(MediaFormat.KEY_HEIGHT);
		        stopCodec();
	...
	public static int getCapbility() {
		...
		for (int i = 0; i < mediaCodecListCount; i++) {
			...
			for (final String type : mediaCodecInfo.getSupportedTypes()) {
				...
				for (final int colorFormat : codecCapabilities.colorFormats) {
					Log.d( LOG_TAG, "Color Format:" + " " + colorFormat + " "+ colorFormatToString(colorFormat));
				}
				...
				for (final CodecProfileLevel codecProfileLevel : codecCapabilities.profileLevels) {
					...
					Log.d( LOG_TAG, "Codec Profile Level:" + level + " profile:"+ sprofile );
		...
		Log.d( LOG_TAG, "Max profile:" + maxProfile + " "+ avcProfileToString(maxProfile) );
	...
	public static int getAVCLevel() {
		...
		Log.d( LOG_TAG, "Max AVCLevel:" + maxAVCLevel + " "+ avcProfileToString(maxAVCLevel) );
