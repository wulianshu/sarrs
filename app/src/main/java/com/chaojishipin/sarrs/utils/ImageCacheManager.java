...
public class ImageCacheManager {
    // 取运行内存阈值的1/8作为图片缓存
    private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) ChaoJiShiPinApplication.getInstatnce()
            .getSystemService(Context.ACTIVITY_SERVICE)).getMemoryClass() / 8;
    private static final int DISK_MEMORY_SIZE = 1024*1024*10;

    private static final String FLODER_NAME = "images";

    private static ImageLruCache mImageLruCache = new ImageLruCache(MEM_CACHE_SIZE,FLODER_NAME,DISK_MEMORY_SIZE);

    public static ImageLoader  mImageLoder = new ImageLoader(HttpManager.getInstance().mQueue,mImageLruCache);
